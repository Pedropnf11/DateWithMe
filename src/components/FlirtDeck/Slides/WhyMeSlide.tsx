...
export default function WhyMeSlide({ data, compact }: WhyMeSlideProps) {
    return (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: compact ? 16 : 28 }}>
            <div>
                <div style={{ fontSize: 10, letterSpacing: "0.3em", color: "rgba(255,80,120,0.7)", textTransform: "uppercase", marginBottom: 6 }}>Pontos Fortes</div>
                <h2 style={{ fontSize: compact ? 22 : 30, color: "#fff", margin: 0, fontWeight: "bold", letterSpacing: "-0.02em" }}>Em mim </h2>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: compact ? 10 : 16, flex: 1 }}>
                {data.map((reason, i) => (
                    <div key={i} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: compact ? "12px 16px" : "18px 20px" }}>
                        <div>
